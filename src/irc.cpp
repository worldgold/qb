...
// Copyright (c) 2011-2012 EB3Coin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #EB3CoinTEST0\r");
            Send(hSocket, "WHO #EB3CoinTEST0\r");
        } else {
            // randomly join #EB3Coin00-#EB3Coin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #EB3Coin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #EB3Coin%02d\r", channel_number).c_str());
        }
